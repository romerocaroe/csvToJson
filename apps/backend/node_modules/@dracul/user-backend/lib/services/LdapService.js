...
async function isLdapAuthEnable() {
  try {
    ...
    _loggerBackend.DefaultLogger.error(error);
  }
}
...
async function getLdapVar(varName) {
  try {
    const ldapVariable = await (0, _settingsBackend.SettingCache)(varName);
...
function loginAsAdmin(adminUser, adminPass) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ldapClient.bind( // `cn=${LDAP_ADMIN}, dc=snd, dc=int`,
      `cn=${LDAP_ADMIN}, ${LDAP_DN}`, LDAP_PASS, error => {
        if (error) {
...
async function getLocalUserOrCreate(userLdapInfo) {
  ...
  let user = await (0, _UserService.findUserByUsername)(userLdapInfo.username);

  if (!user) {
    try {
      const ROLE_NAME = await determineUserRoleByLdapGroup(userLdapInfo.groupName);
      const role = await (0, _RoleService.findRoleByName)(ROLE_NAME);

      if (!role) {
        _loggerBackend.DefaultLogger.error(`ROLE ${ROLE_NAME} not found `);
      ...
      user = await (0, _UserService.createUser)(userLdapInfo);
      return user;
    } catch (error) {
      _loggerBackend.DefaultLogger.error(`Error while trying to create user: '${error}'`);
...
async function authLdapAndGetUser(username, decodedPassword) {
  try {
    ...
    if (userInfo.password === decodedPassword) {
      ...
      _loggerBackend.DefaultLogger.error(`passwords don't match '${userInfo.password}' vs '${decodedPassword}'`);
    ...
    _loggerBackend.DefaultLogger.error(`authLdapAndGetUser error: '${message}' `);
