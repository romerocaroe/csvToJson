...
var _default = {
  Query: {
    ...
    myGroups: (_, {}, {
      ...
      if (!user) throw new _apolloServerExpress.AuthenticationError("Unauthenticated");
      if (!rbac.isAllowed(user.id, _permissions.SECURITY_GROUP_SHOW)) throw new _apolloServerExpress.ForbiddenError("Not Authorized");
      return (0, _GroupService.fetchMyGroups)(user.id);
    },
    group: (_, {
      id
    }, {
    ...
    groupByName: (_, {
      ...
      if (!user) throw new _apolloServerExpress.AuthenticationError("Unauthenticated");
      if (!rbac.isAllowed(user.id, _permissions.SECURITY_GROUP_SHOW)) throw new _apolloServerExpress.ForbiddenError("Not Authorized");
      return (0, _GroupService.findGroupByName)(name);
    },
    groupsPaginate: (_, {
      limit,
      pageNumber,
      ...
      if (!rbac.isAllowed(user.id, _permissions.SECURITY_GROUP_SHOW)) throw new _apolloServerExpress.ForbiddenError("Not Authorized");
      return (0, _GroupService.paginateGroup)(limit, pageNumber, search, orderBy, orderDesc, myGroups ? user.id : null);
    }
  },
  Mutation: {
    groupCreate: (_, {
